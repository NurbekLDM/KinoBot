...
async function handleMandatoryChannels(chatId) {
  try {
    ...
    if (channels.length === 0) {
      await bot.sendMessage(
        chatId,
        "<b>🟩 Majburiy kanallar ro'yxati bo'sh</b>",
        {
          parse_mode: "HTML",
        }
      );
    ...
    await bot.sendMessage(
      chatId,
      "⚠️ Kanallar ro'yxatini olishda xatolik yuz berdi!"
    );
  }
}
...
async function handleJoinRequestChannels(chatId) {
  try {
    ...
    if (channels.length === 0) {
      await bot.sendMessage(
        chatId,
        "<b>📋 Zayavka kanallari ro'yxati bo'sh</b>",
        {
          parse_mode: "HTML",
        }
      );
...
async function startServer() {
  try {
    ...
    if (USE_WEBHOOK) {
      // Webhook rejimi
      if (!WEBHOOK_URL) {
        throw new Error("WEBHOOK_URL environment variable majburiy");
      }
      ...
      await bot.setWebHook(`${WEBHOOK_URL}/webhook/${API_KEY}`);
      console.log("Webhook o'rnatildi ✅");
      
      const PORT = process.env.PORT || 3000;
      app.listen(PORT, () => {
        console.log(`🚀 Server ${PORT} portda ishlamoqda (webhook rejimi)`);
      });
    } else {
      // Polling rejimi
